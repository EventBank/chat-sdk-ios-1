...
+(NSNumber *) messageContentHeight: (id<PElmMessage>) message maxWidth: (float) maxWidth {
    return @([self getText: message.text heightWithFont:[UIFont systemFontOfSize:BChatSDK.config.messageTextFont.pointSize] withWidth:[self messageContentWidth:message maxWidth:maxWidth].floatValue]);
}
...
+(float) getText: (NSString *) text heightWithWidth: (float) width {
    return [text boundingRectWithSize:CGSizeMake(width, CGFLOAT_MAX)
                           ...
                           attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:BChatSDK.config.messageTextFont.pointSize]}
...
+(float) textWidth: (NSString *) text maxWidth: (float) maxWidth {
    if (text) {
        UIFont * font = [UIFont systemFontOfSize:BChatSDK.config.messageTextFont.pointSize];
        if (font) {
